...
class EventCreatePage extends BaseClass {
    ...
    async renderEvent() {
        ...
        let host = window.localStorage.getItem('userId');
        //User elements for the checkboxes
        if (users) {
            for (let rsvp of users) {
                rsvpHTML += `<tr><td>${rsvp.firstName} ${rsvp.lastName}</td>`;
                if (host === rsvp.userId) {
                    rsvpHTML += `<td><input type="checkbox" name="invited" id="${rsvp.userId}" value="${rsvp.userId}" checked></td></tr>`;
                }
                else {
                    rsvpHTML += `<td><input type="checkbox" name="invited" id="${rsvp.userId}" value="${rsvp.userId}"></td></tr>`;
                };
        }
